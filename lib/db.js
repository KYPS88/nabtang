...
async function sb(path, { method = 'GET', body, headers = {} } = {}) {
  // รองรับกุญแจ Supabase ทั้ง 2 ยุค: แบบใหม่ (sb_secret_...) ใช้ header apikey อย่างเดียว
  // แบบเก่า (service_role เป็น JWT ขึ้นต้น eyJ) ต้องส่ง Authorization: Bearer ด้วย
  const key = CFG.key();
