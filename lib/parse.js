...
const CAT_KEYWORDS = {
  food:   ['ข้าว','ก๋วยเตี๋ยว','อาหาร','กาแฟ','ชา','นม','ขนม','เค้ก','หมูกระทะ','ชาบู','บุฟเฟ่','ปิ้งย่าง','ส้มตำ','พิซซ่า','เบอร์เกอร์','ไก่ทอด','กับข้าว','มื้อ','เที่ยง','อาหารเช้า','อาหารเย็น','ร้านอาหาร','kfc','mk','โจ๊ก','ผลไม้','น้ำ','เครื่องดื่ม','คาเฟ่','เครป','ชานม','ชาไข่มุก','หมูปิ้ง','ลูกชิ้น','ไอติม','ไอศกรีม','โดนัท','เบเกอรี่','ปาท่องโก๋','ข้าวเหนียว','หมี่','ผัด','ต้ม','แกง','ยำ'],
...
function parseEntry(text) {
  ...
  // "รับ 1000" (คำว่า รับ เดี่ยว ๆ นำหน้า) นับเป็นรายรับด้วย — แต่ไม่ใช้ includes เพราะจะชนคำอย่าง "ค่าปรับ"
  const isIncome = plusIncome || note === 'รับ' || note.startsWith('รับ ') || INCOME_KEYWORDS.some(w => note.includes(w));
  if (isIncome) {
